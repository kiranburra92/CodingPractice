 ...
 * "ccc" does not match the pattern because {a -> c, b -> c, ...} is not a permutation, since a and b map to the same
 * letter.
 *
 * Example 2:
 *
...
public class FindandReplacePattern {
    ...
    private boolean match(String s, String pattern){
        ...
        for(int i = 0; i< s.length() ; i++) {
            ...
            if(!forward.containsKey(m)) forward.put(m, p);
            if(!backward.containsKey(p)) backward.put(p,m);
