package com.burra.practice.blind75


/**
 * There is an integer array nums sorted in ascending order (with distinct values).

Prior to being passed to your function, nums is possibly rotated at an unknown pivot index k (1 <= k < nums.length)
such that the resulting array is [nums[k], nums[k+1], ..., nums[n-1], nums[0], nums[1], ..., nums[k-1]] (0-indexed).
For example, [0,1,2,4,5,6,7] might be rotated at pivot index 3 and become [4,5,6,7,0,1,2].

Given the array nums after the possible rotation and an integer target, return the index of target if it is in nums, or
-1 if it is not in nums.

You must write an algorithm with O(log n) runtime complexity.



Example 1:

Input: nums = [4,5,6,7,0,1,2], target = 0
Output: 4
Example 2:

Input: nums = [4,5,6,7,0,1,2], target = 3
Output: -1
Example 3:

Input: nums = [1], target = 0
Output: -1


Constraints:

1 <= nums.length <= 5000
-104 <= nums[i] <= 104
All values of nums are unique.
nums is an ascending array that is possibly rotated.
-104 <= target <= 104
 */
class SearchinRotatedSortedArray {
    fun search(nums: IntArray, target: Int): Int {
        val pivot = findAcutualArrayEnding(nums)
        if (pivot < 0) return binarySearch(nums, 0, nums.size - 1, target)
        if (nums[pivot] == target) return pivot
        return if (target < nums[0]) binarySearch(nums, pivot + 1, nums.size - 1, target)
        else binarySearch(nums, 0, pivot - 1, target)
    }

    private fun binarySearch(nums: IntArray, i: Int, j: Int, target: Int): Int {
        var start = i
        var end = j
        while (start <= end) {
            var mid = (start + end) / 2
            if (nums[mid] == target) return mid
            else if (nums[mid] < target) start = mid + 1
            else end = mid - 1
        }
        return -1
    }

    private fun findAcutualArrayEnding(nums: IntArray): Int {
        var start = 0
        var end = nums.size - 1

        while (start <= end) {
            val mid = start + (end - start) / 2
            if (mid + 1 < nums.size && nums[mid] > nums[mid + 1]) {
                return mid
            }
            if (nums[mid] < nums[start]) end = mid - 1
            else start = mid + 1
        }
        return -1
    }
}

fun main() {
    val searchinRotatedSortedArray = SearchinRotatedSortedArray()
    searchinRotatedSortedArray.search(intArrayOf(8, 9, 2, 3, 4), 9)
}